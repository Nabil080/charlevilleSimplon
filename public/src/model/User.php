...
\r
            ...
            Bienvenue à Simplon ! Dernière étape pour votre candidature !   \r
\r

            Veuillez cliquer sur ce lien pour valider votre compte : http://localhost/projet_dev_v2/index.php?action=registerPage&token=$token \r
\r
    ...
    public function sendMailValidationCandidature($user_id, $promo_id)
    {
        ...
        if (isset($user) && !empty($user)) {
            ...
            $message = "Bonjour $user->user_surname,\r
\r

            Votre candidature pour la promotion $promo->name a bien été prise en compte ! Pour rappel, elle début le $promo->start.   \r
\r

            Voici les informations de votre candidature, n'hésitez pas à les modifier sur votre profil s'ils sont incorrects : \r

            Nom : $user->user_name \r

            Prénom : $user->user_surname \r

            Email : $user->user_email \r

            Numéro de téléphone : $user->user_phone \r

            Adresse : $user->user_adress \r

            Numéro pôle Emploi : $user->user_id_poleEmploi \r
\r
...
\r
                ...
                Pour changer votre mot de passe, veuillez cliquer sur le lien ci-dessous : \r
\r

                http://localhost/projet_dev_v2/index.php?action=resetPasswordForm&token=$token \r
\r

                Si vous n'êtes pas à l'origine de cette demande, il vous suffit d'ignorer ce message. \r
\r
    ...
    public function getFilteredLearnersAndFormatorsNumber(
        $filters = null,
        $execute = null,
    ): int {
        $filters = $filters === null ? '' : "AND $filters";
    ...
    public function getFilteredCandidatesNumber(
        $filters = null,
        $execute = null,
    ): int {
        $filters = $filters === null ? '' : "AND $filters";
    ...
    public function getFilteredCompaniesNumber(
        $filters = null,
        $execute = null,
    ): int {
        $filters = $filters === null ? '' : "AND $filters";
