...
<section id="modal-update-<?=$user->id?>" data-modal-placement="center" tabindex="-1" aria-hidden="true" class="backdrop:brightness-50 fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] md:h-full">
    <div class="relative w-full h-fit max-w-md md:h-auto">
        <div class="relative bg-main-white border-main-red border-2 rounded-md text-main-red">
            <button type="button" class="absolute top-3 right-2.5 text-main-red bg-transparent rounded-md text-sm p-1.5 ml-auto inline-flex items-center hover: hover:border border-main-red" data-modal-hide="modal-update-<?=$user->id?>">
                <svg data-modal-hide="modal-update-<?=$user->id?>" aria-hidden="true" class="w-5 h-5 text-main-light" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
            ...
            <div id="co" class="px-6 py-6 lg:px-8">
                ...
                <form class="space-y-6" action="?action=updateUserPersonnalInfos" method="post">
                    ...
                    <div>
                        ...
                        <input type="text" name="surname" class=" border text-sm rounded-md block w-full p-2.5 border-main-red text-black" value="<?=$user->surname?>" required>
                    </div>
                    <div>
                        <label for="name" class="block mb-2 text-sm font-medium  text-main-red">Nom</label>
                        <input type="text" name="name" class=" border text-sm rounded-md block w-full p-2.5 border-main-red text-black" value="<?=$user->name?>" required>
                    </div>
                    <div>
                        <label for="adress" class="block mb-2 text-sm font-medium  text-main-red">Adresse</label>
                        <input type="text" name="adress" class=" border text-sm rounded-md block w-full p-2.5 border-main-red text-black" value="<?=$user->adress?>" required>
                    </div>
                    <div>
                        <label for="email" class="block mb-2 text-sm font-medium  text-main-red">Email</label>
                        <input type="email" name="email" class=" border text-sm rounded-md block w-full p-2.5 border-main-red text-black" value="<?=$user->email?>" required>
                    </div>
                    <div>
                        <label for="phone" class="block mb-2 text-sm font-medium  text-main-red">Téléphone</label>
                        <input type="tel" name="phone" class=" border text-sm rounded-md block w-full p-2.5 border-main-red text-black" pattern="[0][0-9]{9}" placeholder="07256533**" value="<?=$user->phone?>" required>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="birth_place" class=" border text-sm rounded-md block w-full p-2.5 border-main-red text-black" value="<?=$user->birth_place?>" required>
                    </div>
                    <div>
                        <label for="nationality" class="block mb-2 text-sm font-medium  text-main-red">Nationalité</label>
                        <input type="text" name="nationality" class=" border text-sm rounded-md block w-full p-2.5 border-main-red text-black" value="<?=$user->nationality?>" required>
                    </div>
                    <?php }
