...
<section id="modal-update" data-modal-placement="center" tabindex="-1" aria-hidden="true" class="hidden grid place-content-center w-full h-full bg-black bg-opacity-50 backdrop:brightness-50 fixed top-0 left-0 right-0 z-50 p-4 overflow-x-hidden overflow-y-auto md:inset-0  md:h-full">
    <div class="relative w-full h-fit max-w-md md:h-auto">
        <div class="relative bg-main-white border-main-red border-2 rounded-md text-main-red">
            <button type="button" class="absolute top-3 right-2.5 text-main-red bg-transparent rounded-md text-sm p-1.5 ml-auto inline-flex items-center hover: hover:border border-main-red" data-modal-hide="modal-update">
                <svg data-modal-hide="modal-update" data-modal-target="modal-update" aria-hidden="true" class="w-5 h-5 text-main-light" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
            ...
            <div id="co" class="px-6 py-6 lg:px-8">
                ...
                <form class="space-y-6" action="?action=updatePromotion" method="post" target="_blank">
                        ...
                        <select multiple name="formators[]" id="formators" class=" border text-sm rounded-md block w-full p-2.5 border-main-red text-main-red">
                            <option disabled selected value> Formateurs(non obligatoire) </option>
                                <?php foreach ($formators as $formator) { ?>
                                        <option value="<?= $formator->user_id ?>"><?= "$formator->user_name $formator->user_surname"?></option>
                                <?php
                            } ?>
                        </select>
                    </div>
                    <div>
                        ...
                        <input type="date" name="start" id="start-input" class=" border text-sm rounded-md block w-full p-2.5 border-main-red text-main-red" value="" required>
                    </div>
                    <div>
                        <label for="end" class="block mb-2 text-sm font-medium  text-main-red">Date de fin</label>
                        <input type="date" name="end" id="end-input" class=" border text-sm rounded-md block w-full p-2.5 border-main-red text-main-red" value="" required>
                    </div>
