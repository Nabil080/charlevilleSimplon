...
<nav id="navbar" class="w-[100vw] fixed top-0 bg-opacity-30 shadow-[3px_5px_10px] bg-gray-400 text-main-white shadow-[#1111114d] blur-[0.2px] left-0 z-40 transition-all duration-[0.3s]">
    <div class="grid grid-cols-2 transition-all duration-[0.2s] items-center  border-t-[4px] md:grid-cols-[20%_80%] border-main-red">
        <a href="index.php"><img src="assets/img/simplonblanc.png" alt="Logo Simplon" class="w-[150px]" /></a>
        <i onclick="changeNavFunction()" class="fa-solid fa-bars text-[39px] justify-self-end text-main-white mr-6 sm:mr-8 mb-1 md:!hidden hiddenIcon"></i>
        ...
        <div class="changeNav hidden  md:grid blur-[0.2px] col-start-1 col-end-3 md:col-start-auto md:justify-self-end md:mr-2 lg:mr-4 xl:mr-6 translate-x-[80vw] md:translate-x-0 duration-[0.4s] ease-out transition-all ">
            ...
            <div class="grid grid-cols-[25%_60%] sm:mx-auto sm:w-[40%] items-center pt-8 gap-8 md:mr-4 md:pt-0 md:inline-flex md:w-full md:gap-2">
                        ...
                        <ul class="z-50 relative py-2 md:py-0 px-2 md:px-0 text-sm leading-9 sm:leading-[60px] divide-y border-2 border-main-red divide-main-red rounded-[4px]" aria-labelledby="dropdownDefaultButton">
                            <?php foreach ($formations as $formation) { ?>
                                <li class="md:hover:bg-gray-400">
                                    <a href="?action=formationPage&id=<?= $formation->id ?>" class="block px-4 py-2 text-[22px] sm:text-[32px] md:text-[20px]"><?= $formation->name ?></a>
                                </li>
                            <?php } ?>
                        </ul>
                    </div>
                ...
                <?php if (isset($_SESSION['user'])) { ?>
                    <div>
                        <button data-dropdown-toggle="dropdownAccount" class="focus:outline-none rounded-[4px] text-[24px] sm:text-[36px] md:text-[20px] xl:text-[24px] text-center inline-flex items-center" type="button">Compte
                            <svg class="w-8 h-8 ml-2" aria-hidden="true" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7">
                                </path>
                            </svg>
                        </button>
                        <div id="dropdownAccount" class="z-50 border-main-lightgray min-w-[300px] hidden bg-gray-400 bg-opacity-80  text-main-white blur-[0.3px] rounded-lg shadow text-center w-fit mr-4 pl-0">
                            <ul class="z-50 relative py-2 md:py-0 px-2 md:px-0 text-sm leading-9 sm:leading-[60px] divide-y border-2 border-main-red divide-main-red rounded-[4px]" aria-labelledby="dropdownAccountDefaultButton">
                                ...
                                <?php if (isset($_SESSION['user']) && $_SESSION['user']->role_id == 1) { ?>
                                    <li class="md:hover:bg-gray-400">
                                        <a href="?action=crudLearnerPage" class="block px-4 py-2 text-[22px] sm:text-[32px] md:text-[20px]">Admin</a>
                                    </li>
                                <?php } ?>
                                ...
                                <?php if (isset($_SESSION['user']) && $_SESSION['user']->role_id == 4){ ?>
                                    <li class="md:hover:bg-gray-400">
                                        <a href="?action=promotionPage" class="block px-4 py-2 text-[22px] sm:text-[32px] md:text-[20px]">Ma promo</a>
                                    </li>
                                <?php } ?>
                ...
                ?>
                    <button type="button" id="connexion" class="text-[24px] sm:text-[36px] md:text-[20px] xl:text-[24px] text-left md:text-center cursor-pointer" data-modal-target="login-modal" data-modal-toggle="login-modal">Connexion</button>
                <?php
                }
                ?>
            </div>
