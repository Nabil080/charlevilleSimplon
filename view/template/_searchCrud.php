...
<div class="bg-white relative shadow-md sm:rounded-lg overflow-hidden">
    <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-2 pb-2">
        ...
        <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 md:pr-8 flex-shrink-0">
            ...
            <button <?php if( str_contains($_GET['action'],'crudProjetPage')){ ?>onclick="window.location.href='?action=addProject'"<?php } ?>data-modal-target="modal-add" data-modal-toggle="modal-add" type="button" class="<?php if (isset($candidatPage) && $candidatPage == 1) { ?>hidden <?php } ?>flex items-center bg-main-red text-white justify-center font-medium rounded-lg 
                    text-sm px-4 py-2">
