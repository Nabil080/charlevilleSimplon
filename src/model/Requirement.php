...
class RequirementRepository extends ConnectBdd
{
    ...
    public function getRequirementByFormation($formation_id): array
    {
        ...
        $req = $this->bdd->prepare("SELECT `requirement_name` FROM `requirement` AS r INNER JOIN `formation_requirement` AS fr ON r.requirement_id = fr.requirement_id WHERE r.formation_id = ?");
