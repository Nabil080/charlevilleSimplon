...
function closeAllDropdowns() {
  ...
  document
    ...
    .forEach((toggle) => toggle.classList.add("rounded-b-lg"));
}
...
const formationCheckboxes = document.querySelectorAll(
  "#formation-dropdown input"
);
...
function loadProjects(projets, number) {
  ...
  for (page = 1; page <= pageCount; page++) {
    if (
      page > currentPage - paginationRange &&
      page < currentPage + paginationRange
    ) {
      if (page == currentPage) {
